...
void main() {
  test('parse ios deeplink exception', () {
    ...
    final redirectErrorDescription =
        'Error Domain= Code=0 "Redirection to URL with a scheme that is not HTTP(S)" UserInfo={_WKRecoveryAttempterErrorKey=<WKReloadFrameErrorRecoveryAttempter: 0x600002534ea0>, NSErrorFailingURLStringKey=$correctUrl, NSLocalizedDescription=Redirection to URL with a scheme that is not HTTP(S)}';

    final exceptionParser = IOSExceptionParser(redirectErrorDescription);

    assert(exceptionParser.getUrlFromErrorDescription() == correctUrl);
  });
}
